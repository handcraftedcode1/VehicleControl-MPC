        ...
        if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    // 2) Translation world (x,y) to vehicle (x,y)
                    for (int i = 0; i < ptsx.size(); i++) {
                        double x_translated =
                        (ptsx[i] - px) * cos(-psi) - (ptsy[i] - py) * sin(-psi);
                        double y_translated =
                        (ptsx[i] - px) * sin(-psi) + (ptsy[i] - py) * cos(-psi);
                        ptsx_vector(i) = x_translated;
                        ptsy_vector(i) = y_translated;
                    }
                    
                    // Calculate a polynomail that approximates the optimal vehicle path
                    // along waypoints
                    auto coeffs = polyfit(ptsx_vector, ptsy_vector, 3);
                    
                    // Calculate the crosstrack & orientation errors
                    ...
                    double dt = 0.100;  // latency is 100ms
                    double Lf = 2.67;
                    
                    // Predict state considering a latency of dt
                    double pred_px = v * cos(0) * dt;
                    double pred_py = v * sin(0) * dt;
