...
public:
    ...
    void operator()(ADvector &fg, const ADvector &vars) {
        ...
        for (int t = 0; t < N - 1; t++) {
            ...
            fg[0] += 200 * CppAD::pow(vars[delta_start + t] * vars[a_start + t], 2);
            fg[0] += CppAD::pow(vars[delta_start + t] * vars[v_start + t], 2);
        }
        
        // Minimize the value gap between sequential actuations
        for (int t = 0; t < N - 2; t++) {
            // punish strong steering angles
            fg[0] += 1000 *
            CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
            
            // punish steering + acceleration at same time
            fg[0] += 1000 *
            CppAD::pow((vars[delta_start + t] - vars[delta_start + t + 1]) *
                       (vars[a_start + t + 1] - vars[a_start + t]),
                       2);
        }
        ...
        for (int t = 1; t < N; t++) {
            ...
            // Now we have a polynomial of third order!!
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
    ...
    // Collect the top optimial steering angle and acceleration
    result.push_back(solution.x[delta_start]);
    result.push_back(solution.x[a_start]);
    
    // now add the (x,y) values of the predicted trajectory
    // as (x1,y1,x2,y2,x3,y3,..... up to N-1)
    for (int i = 0; i < N - 1; i++) {
